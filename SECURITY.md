...
- **Mitigation:** Poisson-distributed "Cover Traffic" and fixed block "Message Padding".
- **Evidence**: `test_traffic_analysis_padding_enforcement` (Passed).

### 2. Active Network Adversary (MITM)
**Capability:** Injecting or modifying packets.
- **Mitigation:** Ed25519 identity pinning and Safety Numbers.
- **Evidence**: `test_identity_pinning_and_mitm_rejection` (Passed).
...
- **Metadata:** Absolute anonymity requires routing the protocol via Tor (`proxy_url`). IP-hiding is not a native crypto-layer feature.
