...
use sibna_core::{Config, HandshakeRole, ProtocolError, SecureContext};
...
async fn test_offensive_require_safety_numbers_enforcement() {
    ...
    assert!(alice_keystore
        .verify_or_pin_peer_key(&peer_id, &peer_id)
        .is_ok());
    ...
    assert!(
        ...
        "Session creation failed even after verification!"
    );
}
...
async fn test_offensive_identity_swap_detection() {
    ...
    alice_ks
        .verify_or_pin_peer_key(b"bob_user", &bob_id.ed25519_public)
        .unwrap();
    drop(alice_ks);
...
async fn test_concurrent_replay_race_condition() {
    ...
    assert_eq!(
        success_count, 1,
        "Exactly one decryption should succeed; found {}",
        success_count
    );
    assert_eq!(
        replay_detected_count,
        num_attackers - 1,
        "All other attempts must be detected as replays"
    );
}
...
async fn test_fuzz_active_tampering_resilience() {
    ...
    for i in 0..20 {
        ...
        assert!(
            result.is_err(),
            "Tampered ciphertext #{} was accepted! (Index: {}, Size: {})",
            i,
            flip_idx,
            ct.len()
        );
        ...
        assert_eq!(
            valid_dec, b"HEARTBEAT",
            "Session state corrupted by malformed input!"
        );
    }
}
...
async fn test_high_latency_extreme_reordering() {
    ...
    for i in 0..20 {
        messages.push(
            alice
                .encrypt(format!("REORDER_{}", i).as_bytes(), b"")
                .unwrap(),
        );
    }
    ...
    let indices = [
        19, 18, 0, 1, 10, 5, 6, 7, 8, 9, 2, 3, 4, 11, 12, 13, 14, 15, 16, 17,
    ];
...
async fn test_offensive_timestamp_time_travel() {
    ...
    assert!(
        result.is_err(),
        "Protocol accepted a message from the far future!"
    );
}
...
async fn test_offensive_header_dh_corruption() {
    ...
    assert!(
        result.is_err(),
        "Tampered DH public key in header bypassed authentication!"
    );
}
