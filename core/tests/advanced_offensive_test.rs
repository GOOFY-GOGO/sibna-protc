...
use sibna_core::crypto::{CryptoError, CryptoHandler};
use sibna_core::{Config, HandshakeRole, SafetyNumber, SecureContext};
...
fn test_identity_pinning_and_mitm_rejection() {
    ...
    assert!(
        result.is_err(),
        "Key change must be rejected as an identity mismatch (MITM)"
    );
}
...
fn test_safety_number_consistency() {
    ...
    assert_eq!(
        sn1.as_string(),
        sn2.as_string(),
        "Safety numbers must be stable regardless of key order"
    );
}
