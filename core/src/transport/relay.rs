...
//! Relay Transport — SOCKS5/Tor Anonymity v3.0.1 - SECURITY HARDENED
//!
...
impl RelayClient {
    ...
    pub fn new(server_url_str: &str, proxy_url_str: Option<&str>) -> ProtocolResult<Self> {
        let server_url = Url::parse(server_url_str).map_err(|_| ProtocolError::InvalidMessage)?;
        ...
        if let Some(proxy_str) = proxy_url_str {
            ...
            let proxy = Proxy::all(proxy_str).map_err(|e| {
    ...
    pub async fn send_envelope(&self, envelope_json: &str) -> ProtocolResult<()> {
        ...
        if res.status().is_success() {
            ...
            warn!(
                "RELAY_SECURITY: Envelope rejected with status {}",
                res.status()
            );
            Err(ProtocolError::HandshakeFailed)
        }
